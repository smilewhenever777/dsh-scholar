...
interface XrefEntry { type: number; f2: number; f3: number }
interface ParsedCmap { map: Record<string, string>; twoByte: boolean }
type PdfObjectGetter = (num: string | number | undefined) => ParsedPdfObject | null
...
function findStreamEnd(body: string, startIdx: number): { end: number; data: string } {
...
function extractTextOperations(content: string): PdfTextRun[] {
    ...
    if (m[1] !== undefined) {
      ...
      while ((pm = partRe.exec(inner)) !== null) {
        if (pm[1] !== undefined) parts.push({ text: decodePdfString(pm[1]) })
          ...
          parts.push({ text: s })
        } else if (pm[3] !== undefined) parts.push({ gap: parseFloat(pm[3]) })
      }
      let buf = ''
      let gapSum = 0
...
function parseCmap(cmapText: string): { map: Record<string, string>; twoByte: boolean } {
  const map: Record<string, string> = {}
  let twoByte = false
  const key = (num: number, width: number): string => num.toString(16).toUpperCase().padStart(width, '0')
  ...
  while ((m = bfcharRe.exec(cmapText)) !== null) {
    ...
    while ((p = pairRe.exec(block)) !== null) {
      ...
      if (source.length > 2) twoByte = true
...
function collectPageNums(latin1: string) {
  ...
  function undoPredictor(data: string, predictor: number, columns: number): string {
  ...
  function extractFilters(dictPart: string): string[] {
    ...
    } else if (f2?.[1] !== undefined) filters.push(f2[1])
    return filters
  }

  function parseObjectBody(raw: string): ParsedPdfObject {
  ...
  function parseXrefStreamEntries(node: ParsedPdfObject): Map<number, XrefEntry> {
    ...
    let indexPairs: Array<[number, number]>
    ...
    if (dpMatch) {
      ...
      if (pm?.[1] !== undefined && parseInt(pm[1], 10) >= 2) decoded = undoPredictor(decoded, parseInt(pm[1], 10), cm?.[1] !== undefined ? parseInt(cm[1], 10) : 1)
    }
    ...
    for (const pair of indexPairs) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        p += rowLen
        let q = 0
        const read = (len: number): number => { let v = 0; for (let k = 0; k < len; k++) v = v * 256 + row[q + k]!; q += len; return v }
  ...
  function loadRawByOffset(num: string | number, offset: number): string | null {
  ...
  if (smAll !== null && smAll.length > 0) {
    ...
    if (xrefOffsetMatch?.[1] === undefined) throw new Error('PDF 结构无法解析（startxref 无效）')
    ...
    if (/^\s*xref\b/.test(at)) {
      ...
      if (tr?.[1] !== undefined) trailerDict = tr[1]
      ...
      if (headNumMatch) {
        ...
        let xrefNode: string | null | undefined = objects[xrefNum]
  ...
  // ---------- ObjStm 对象流展开与对象解析 ----------
  const objstmObjects: Record<string, string> = {}
  const expandedStms = new Set<number>()

  function expandObjStm(sn: number): void {
  ...
  function getObject(num: string | number | undefined): ParsedPdfObject | null {
    if (!num) return null
    let raw = objects[num]
  ...
  function resolveRef(dict: string, key: string): string | null {
    const re = new RegExp('/' + key + '\\s+(\\d+)\\s+\\d+\\s+R')
    const mm = dict.match(re)
    return mm?.[1] ?? null
  }
  function resolveMultiRef(dict: string, key: string): string[] {
    const out: string[] = []
    ...
    if (mm) {
      ...
      let r
      while ((r = refRe.exec(mm[1]!)) !== null) if (r[1] !== undefined) out.push(r[1])
      return out
    }
...
function extractPageTexts(
  pageNums: string[],
  getObject: PdfObjectGetter,
  resolveRef: PdfRefResolver,
  resolveMultiRef: PdfMultiRefResolver,
  onPage: ((progress: PdfProgress) => void) | null = null,
): string[] {
  const fontMaps: Record<string, ParsedCmap | null> = {}
  function getFontMap(fontRef: string | null | undefined): ParsedCmap | null {
...
function extractPdfText(latin1: string, onPage?: (progress: PdfProgress) => void): string {
...
function extractPdfStats(latin1: string): PdfStats {
  if (latin1.slice(0, 5) !== '%PDF-') throw new Error('不是有效的 PDF 文件')
  const state = collectPageNums(latin1)
