...
export function DeepreadLauncher({ open, current, allPapers: allPapersProp, onClose, notify, t }: {
  ...
  const [mode, setMode] = useState<'paper' | 'quick'>('paper');
  ...
  const launch = async () => {
    ...
    try {
      ...
      if (lens && mode === 'paper') focusParts.push(ACADEMIC_LENS);
      const rel = allPapers.filter((p) => ctxIds.has(p.id));
      if (rel.length > 0) focusParts.push('库内相关论文（对比定位，透镜第⑤点引用）：' + rel.map((p) => p.title).join('；'));
      ...
      const r = await api<{ ok: boolean; jobId?: string; detached?: boolean; label?: string; started?: number; skipped?: number }>('/scholar/read/run', {
        method: 'POST',
        body: JSON.stringify({ paperIds: targetIds, mode, focus: focusParts.join('\n') }),
      });
