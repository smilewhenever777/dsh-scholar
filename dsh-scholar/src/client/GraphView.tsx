...
export function GraphView({ t }: { t: TFunc }) {
  ...
  const display = useMemo<KnowledgeGraph | null>(() => {
    ...
    if (!f) return g0;
    const center = g0.nodes.find((n) => n.id.toLowerCase().includes(f) || n.label.toLowerCase().includes(f));
    ...
    return { nodes: g0.nodes.filter((n) => seen.has(n.id)), edges };
  }, [graph, debouncedFilter, depth, showCites]);
