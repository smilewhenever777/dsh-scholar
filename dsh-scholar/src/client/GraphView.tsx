...
import type { GraphEdge, GraphNode, KnowledgeGraph, Paper, PaperCollection } from '../shared/types';
import { api } from './api';
...
const EDGE_KINDS = ['proposes', 'improves', 'extends', 'builds_on', 'compares', 'uses', 'cites', 'derives_from', 'related'] as const;
const PAPER_H = 24;
...
function simulate(graph: KnowledgeGraph, width: number, height: number, warm?: Map<string, Pos>): Map<string, Pos> {
  ...
  for (let iter = 0; iter < iterations; iter++) {
    ...
    // gravity to column + center：弱化两列硬分（0.015→0.005），Obsidian 式
    // 社区聚类感——论文群靠边自然抱团，概念贴着自己的论文
    for (const node of graph.nodes) {
      const p = pos.get(node.id) as Pos;
      const c = vel.get(node.id) as { vx: number; vy: number };
      const col = node.kind === 'paper' ? colPaper : colConcept;
      p.x += (col - p.x) * 0.005 * t;
      p.y += (height / 2 - p.y) * 0.004 * t;
...
export function GraphView({ t }: { t: TFunc }) {
  ...
  useEffect(() => {
    ...
    // 分区着色原料：分区表 + 论文→分区归属（失败静默，着色退中性色）
    void api<{ collections: PaperCollection[] }>('/scholar/collections')
      .then((r) => setCollections(r.collections ?? []))
      .catch(() => {});
    void api<{ papers: Paper[] }>('/scholar/papers?sort=title')
      .then((r) => setPaperList(r.papers ?? []))
      .catch(() => {});
  }, [t]);

  /** 引用边显隐（浅灰文献边 vs 语义边分层开关）。
   *  注意：必须声明在下方 display useMemo 之前——memo 回调在渲染期执行，
   *  放后面会触发 TDZ（Cannot access before initialization）导致插槽崩溃。 */
  const [showCites, setShowCites] = useState(true);
  /** 冗余修剪（Obsidian 过滤器语义，默认开）：叶概念（度=1 的 tag 同步噪声）、
   *  孤立节点、平行边、自环全部隐藏——255 节点视图的主要冗余源 */
  const [prune, setPrune] = useState(true);
  /** cite-sync 全库扫描耗时较长，按钮需有等待态 */
  const [citeBusy, setCiteBusy] = useState(false);
  const [citeAdded, setCiteAdded] = useState<number | null>(null);

  // filter → local graph around ANY matching node (concept OR paper), BFS
  // depth-adjustable like Obsidian's local graph; hides cites edges when toggled off
  const display = useMemo<KnowledgeGraph | null>(() => {
    if (!graph) return null;
    let g0: KnowledgeGraph = graph;
    if (!showCites) {
      // 隐藏引用边时，"只剩 cites 边"的节点同步隐藏（Obsidian 过滤器语义）；
      // 原本就孤立的节点保持显示，不改变无 cites 时的行为
      const edges = graph.edges.filter((e) => e.kind !== 'cites');
      const kept = new Set<string>();
      const hadAny = new Set<string>();
      for (const e of graph.edges) { hadAny.add(e.source); hadAny.add(e.target); }
      for (const e of edges) { kept.add(e.source); kept.add(e.target); }
      g0 = { nodes: graph.nodes.filter((n) => kept.has(n.id) || !hadAny.has(n.id)), edges };
    }
    if (prune) {
      // 修剪：①概念节点度<2 是 tag 同步噪声（叶概念）②孤立节点 ③自环 ④平行边（同对节点只留一条）
      ...
      const nodes = g0.nodes.filter((n) => (deg.get(n.id) ?? 0) > 0 && (n.kind !== 'concept' || (deg.get(n.id) ?? 0) >= 2));
      const alive = new Set(nodes.map((n) => n.id));
      const dedup = new Set<string>();
      const edges: GraphEdge[] = [];
      for (const e of g0.edges) {
        if (e.source === e.target || !alive.has(e.source) || !alive.has(e.target)) continue;
        const key = e.source < e.target ? `${e.source}|${e.target}` : `${e.target}|${e.source}`;
    ...
    if (!f) return g0;
    // 搜索改为「全图高亮」模式（Obsidian 搜索语义）：不再跳局部图，全图保持原位，
    // 命中节点+邻域点亮、其余淡出。display 始终返回全图，点亮集合由 searchLit memo 计算。
    return g0;
  }, [graph, debouncedFilter, showCites, prune]);

  /** 搜索高亮集合：命中节点 + depth-1 跳邻域；边取两端都在点亮集内的 */
  const searchLit = useMemo(() => {
    const f = debouncedFilter.trim().toLowerCase();
    if (!f || !display) return null;
    const matches = new Set<string>();
    for (const n of display.nodes) {
      if (n.id.toLowerCase().includes(f) || n.label.toLowerCase().includes(f)) matches.add(n.id);
    }
    if (matches.size === 0) return { lit: new Set<string>(), litEdges: new Set<number>(), matched: 0 };
    ...
    const lit = new Set<string>(matches);
    let frontier = [...matches];
    for (let h = 1; h < depth; h++) {
      const next: string[] = [];
      for (const id of frontier) {
        for (const other of adj.get(id) ?? []) {
          if (!lit.has(other)) { lit.add(other); next.push(other); }
        }
      }
      frontier = next;
    }
    ...
    display.edges.forEach((e, i) => {
      if (lit.has(e.source) && lit.has(e.target)) litEdges.add(i);
    });
    return { lit, litEdges, matched: matches.size };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  /** 布局重排的缓动动画：旧位置→新位置 450ms ease-out（Obsidian 的"活的"图感）。
   *  拖拽直接写 posState，本 effect 只在 positions（重排）变化时接管。 */
  const prevPosRef = useRef<Map<string, Pos> | null>(null);
  useEffect(() => {
    ...
    for (const [id, p] of positions) {
      const q = prev.get(id);
      if (q) merged.set(id, { x: q.x, y: q.y });
    }
    posState.current = merged;
    setPositionsTick((v) => v + 1);
    ...
    const step = (now: number) => {
      ...
      for (const [id, target] of positions) {
        const q = prev.get(id);
        if (q) merged.set(id, { x: q.x + (target.x - q.x) * e, y: q.y + (target.y - q.y) * e });
      }
      setPositionsTick((v) => v + 1);
      if (k < 1) raf = requestAnimationFrame(step);
      else posState.current = positions;
    };
  ...
  /** OpenAlex citation sync: adds cites edges only between library papers (idempotent) */
  const runCiteSync = async () => {
    if (citeBusy) return;
    setCiteBusy(true);
    try {
      const r = await api<{ addedEdges: number; scanned: number; resolved: number; missCount: number }>('/scholar/graph/cite-sync', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: '{}',
      });
      ...
      setGraph(g.graph);
      setUnsynced(st.unsynced ?? []);
      setCiteAdded(r.addedEdges);
      setTimeout(() => setCiteAdded(null), 4000);
      setError('');
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
    } finally {
  ...
  const copySyncPrompt = async () => {
    ...
    const text = '请更新知识图谱（kg_extract）：'
      + '\n1. 为这些新保存的论文补充概念节点与语义关系边：' + names
      + '\n2. 概念归一：把同义/中英对照的概念节点合并成一个（如「目标检测」=「object detection」=「检测」只留一个，'
      + '优先保留中文常用名），把旧概念节点上的边改接到合并后的节点，孤立的概念节点删除；'
      + '用 PUT /scholar/graph（mode=append 提交合并结果，被合并节点的边要先改指到保留节点再删除冗余节点）。';
    try {
  ...
  const noMatch = !!filter && !!searchLit && searchLit.matched === 0;
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '8px 10px 6px', display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <button
          type="button"
          onClick={() => void runCiteSync()}
          disabled={citeBusy}
          title={t('graph.citeSyncHint')}
          style={{
            display: 'inline-flex', alignItems: 'center', height: 26, flex: 'none',
            border: '1px solid var(--dsw-alias-border-l2)',
            background: citeBusy ? 'var(--dsw-alias-bg-layer-1, rgba(127,127,127,.06))' : 'transparent',
            color: citeAdded !== null ? T.success : T.secondary,
            borderRadius: 7, padding: '0 9px', cursor: citeBusy ? 'default' : 'pointer',
            fontSize: 11, opacity: citeBusy ? 0.75 : 1,
          }}
        >
          {citeBusy ? t('graph.citeSyncRunning') : citeAdded !== null ? t('graph.citeSyncDone', { n: citeAdded }) : t('graph.citeSync')}
        </button>
        <IconButton
          label={t('graph.showCites')}
          active={showCites}
          onClick={() => setShowCites(!showCites)}
          icon={<Icon d={Icons.link} size={13} />}
        />
        <button
          type="button"
          onClick={() => setPrune((v) => !v)}
          title={t('graph.pruneHint')}
          style={{
            display: 'inline-flex', alignItems: 'center', height: 26, flex: 'none',
            border: `1px solid ${prune ? 'color-mix(in srgb, var(--dsw-alias-state-business-primary, #4d6bfe) 40%, transparent)' : 'var(--dsw-alias-border-l2)'}`,
            background: prune ? 'color-mix(in srgb, var(--dsw-alias-state-business-primary, #4d6bfe) 12%, transparent)' : 'transparent',
            color: prune ? T.business : T.secondary,
            borderRadius: 7, padding: '0 9px', cursor: 'pointer', fontSize: 11,
          }}
        >
          {t('graph.prune')}
        </button>
      ...
      {noMatch && (
        <div style={{ padding: '0 12px 4px', fontSize: 11, color: T.caption }}>{t('graph.noMatch', { q: filter })}</div>
      )}
        ...
        >
          <g transform={`translate(${view.x},${view.y}) scale(${view.scale})`}>
            {display && display.edges.map((e, i) => {
              ...
              const hoverLit = !highlight || highlight.edges.has(i);
              const sLit = !searchLit || searchLit.litEdges.has(i);
              const lit = hoverLit && sLit;
              // Obsidian 视觉：底噪边极淡（0.2），高亮时才全亮+箭头——324 条箭头同时画是主要杂讯源
              const opacity = highlight ? (lit ? 0.9 : 0.05) : sLit ? (lit ? 0.75 : 0.05) : 0.2;
              const strokeWidth = (highlight || searchLit) && lit ? 1.6 : 0.7;
              const showArrow = highlight ? lit : false;
            ...
            {display && display.nodes.map((node) => {
              ...
              const inSearchLit = !searchLit || searchLit.lit.has(node.id);
              const hoverDim = highlight ? (highlight.nodes.has(node.id) ? 1 : 0.16) : 1;
              const dim = (searchLit ? (inSearchLit ? 1 : 0.1) : 1) * hoverDim;
              const d = degree.get(node.id) ?? 0;
              const ideaR = 9 + Math.min(4, d * 0.7); // 想法菱形随度微放大（d 必须先声明）
              /* 分区着色：论文节点按第一个所属分区的颜色着色（Obsidian group 语义） */
              const colOf = isPaper ? collections.find((c) => (nodeMeta.get(node.id) ?? []).includes(c.id)) : undefined;
              const paperFill = colOf
                ? `color-mix(in srgb, ${colOf.color} 26%, var(--dsw-alias-bg-layer-2, #222))`
                : 'var(--dsw-alias-bg-layer-2, #222)';
              const paperStroke = colOf ? `color-mix(in srgb, ${colOf.color} 70%, transparent)` : 'var(--dsw-alias-border-l2)';
              return (
                <g
                  key={node.id}
                  transform={`translate(${p.x},${p.y})`}
                  style={{ cursor: 'pointer', opacity: dim, transition: 'opacity .18s ease' }}
                  onPointerDown={(e) => {
                    e.stopPropagation();
                    setDragNode(node.id);
                    setSelectedId(node.id);
                    // 捕获指针:快速拖动出节点范围也能持续收到 move/up
                    try { (e.currentTarget as Element).setPointerCapture(e.pointerId); } catch { /* 指针已释放(极速拖拽/合成事件)——选中态已生效,拖拽下次按下重建 */ }
                  }}
                  ...
                  {isPaper ? (
                    <>
                      <rect
                        ...
                        fill={paperFill}
                        stroke={isSel ? T.business : paperStroke}
                    ...
                    <>
                      <rect
                        x={-ideaR / 1.41} y={-ideaR / 1.41} width={ideaR / 0.705} height={ideaR / 0.705} rx={2}
                        transform="rotate(45)"
                        fill="color-mix(in srgb, var(--dsw-alias-state-warn-primary, #f5a524) 22%, transparent)"
                        stroke={isSel ? '#f5a524' : 'color-mix(in srgb, var(--dsw-alias-state-warn-primary, #f5a524) 55%, transparent)'}
                        strokeWidth={isSel ? 1.8 : 1}
                      />
                      <text
                        x={p.x > size.w * 0.72 ? -(ideaR + 6) : ideaR + 6}
                        y={3.5} fontSize={9.5}
                    ...
                    <>
                      ...
                      {(() => {
                        /* Obsidian 标签策略：概念标签只在 hover/选中/枢纽(度≥4)/搜索命中时显示——
                           几十个叶概念常亮标签是视觉噪音的主源 */
                        const lit = highlight ? highlight.nodes.has(node.id) : false;
                        const visible = isSel || lit || d >= 4 || (searchLit ? searchLit.lit.has(node.id) : false);
                        return (
                          <text
          ...
          {collections.length > 0 && (
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, marginLeft: 4 }}>
              {collections.slice(0, 8).map((c) => (
                <span key={c.id} title={c.name} style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }}>
                  <span style={{ width: 7, height: 7, borderRadius: 2, background: c.color, display: 'inline-block', border: '1px solid rgba(255,255,255,.25)' }} />
                  {c.name}
                </span>
              ))}
            </span>
          ...
          <div className="sch-fade" style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span aria-hidden style={{
                ...
                transform: selected.kind === 'idea' ? 'rotate(45deg)' : undefined,
                background: selected.kind === 'paper' ? T.business : selected.kind === 'idea' ? '#f5a524' : edgeColor('uses'), flex: 'none',
              }} />
              <span style={{ fontWeight: 600, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {selected.label}
              </span>
              <span style={{ fontSize: 9.5, color: T.caption, flex: 'none' }}>
                {selected.kind === 'paper' ? t('graph.paper') : selected.kind === 'idea' ? t('graph.idea') : t('graph.concept')}
              </span>
            ...
            {selected.kind === 'idea' && (
              <div style={{ marginTop: 7 }}>
                <Btn tone="soft" onClick={() => navBus.go('cards', undefined, selected.id)}>
                  <Icon d={Icons.bulb} size={11} /> {t('graph.viewCard')}
                </Btn>
              </div>
            )}
          </div>
