    ...
    return (
      <div className="sch-scroll sch-fade" style={{ flex: 1, minHeight: 0, overflow: 'auto', padding: '10px 12px 14px', fontSize: 12 }} key={selected.id}>
        ...
        <div style={{ marginTop: 7, display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
          ...
          {(() => {
            const vb = venueBadge(selected);
            return vb ? (
              <span style={{
                fontSize: 10, fontWeight: 600, padding: '1px 7px', borderRadius: 6,
                color: vb.color,
                background: `color-mix(in srgb, ${vb.color} 14%, transparent)`,
                border: `1px solid color-mix(in srgb, ${vb.color} 35%, transparent)`,
              }}>{vb.label}</span>
            ) : null;
          })()}
        ...
        {/* 阅读状态分段控件：想读 → 在读 → 读过 */}
        <div style={{ marginTop: 8, display: 'inline-flex', gap: 4, alignItems: 'center' }}>
