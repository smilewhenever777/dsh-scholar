...
import type { PaperOutcome, QuickOutcome, ReadEngine, ReadOutcome } from './read/engine.js';
import { archiveReadResult } from './read/archive.js';
import { loadSession, appendQA } from './read/session.js';
import { createAsker } from './read/ask.js';
...
export function registerScholarTools(
  ...
  let toolCount = 0;
  // 自有精读引擎与追问器：宿主缺 llm 服务时保持 null（工具调用时报明确错误）
  const readEngine: ReadEngine | null = (ctx as { llm?: unknown }).llm ? createReadEngine(ctx as never) : null;
  const asker = (ctx as { llm?: unknown }).llm ? createAsker(ctx as never) : null;
  ctx.effect(() => {
    const disposers = [
      /* ---------- 1. paper_save ---------- */
      ...
      ctx.tools.register(defineTool({
        ...
        description:
          '归档**外部提供的** HTML 报告到论文库并关联指定论文（用户明确给了现成 HTML 时才用）。'
          + '⚠ 精读论文本身禁止走这个工具——必须用 paper_read（自动归档五段式报告+建追问会话+对比原料）；'
          + '手写报告经此入库会丢追问/对比/模式标记，且会话里绝不存在"deepread 引擎"（已卸载），不要在报告落款里声称用了它。',
        parameters: {
          paper: { type: 'string', required: true, description: '论文 id 或标题关键词' },
          html: { type: 'string', required: true, description: '外部 HTML 报告的完整内容（精读产物不要传这里——用 paper_read）' },
          force: { type: 'boolean', description: '确认为用户明确提供的历史/外部精读样式 HTML，强制归档（绕过拦截；仅当用户明确要求时使用）' },
        },
        output: {
          schema: {
            type: 'object', additionalProperties: false,
        ...
        async execute(args: any) {
          ...
          // 硬拦截：精读样式 HTML 一律拒收——对话 AI 有手写报告归档的惯性（自我模仿强于
          // 指令），提示词拦不住；服务端拒绝+指路是最后防线。用户明确要求时 force=true 放行。
          if (args.force !== true) {
            const html = String(args.html ?? '');
            const markers = ['知识地图', '主动回忆', '回忆问题', '观点与证据表', '思维导图', 'dsh-deepread', '精读报告 · 生成于', '二次精读'].filter((m) => html.includes(m));
            if (markers.length >= 2) {
              return {
                ok: false,
                error: '已拒绝：这是精读报告样式的内容（命中标记：' + markers.slice(0, 3).join('、') + '）。精读论文请直接用 paper_read 工具'
                  + '（mode=paper，自动归档九节报告+创建追问会话+对比原料 sidecar）；手写 HTML 归档会丢失这些能力且格式与库不一致。'
                  + '如用户明确要求归档这份外部 HTML，传 force=true 重试。',
              };
            }
          }
          const paper = candidates[0];
      ...
      /* ---------- 9. paper_read：自有精读引擎（论文 PDF → 五段式报告归档 + summary 回填） ---------- */
      ctx.tools.register(defineTool({
        name: 'paper_read',
        description:
          '用学者工作台自带的精读引擎读一篇论文（或任意本地 txt/md/html 报告文件），完成后自动归档五段式报告并回填论文 summary。'
          + 'mode=paper（默认，学术论文管线：章节感知分段+方法/实验/消融提取+知识地图）或 quick（速读筛文献）。'
          + '用户说"精读/读透这篇/速读"时调用；报告归档后详情页「精读报告」可直接预览。',
        parameters: {
          paper: { type: 'string', description: '论文 id 或标题关键词（二选一：paper 或 path）' },
          path: { type: 'string', description: '本地文件绝对路径（.pdf/.txt/.md/.html；读已归档的精读报告时用）' },
          mode: { type: 'string', description: 'paper（默认） | quick' },
          light: { type: 'boolean', description: '轻量档（长文）：综合直接用分段摘要，跳过全文大调用——快、API 繁忙时稳，精度略降。用户说"轻量精读/快速深读/API 忙随便读一下"时传 true；默认不传（标准深读）' },
          focus: { type: 'string', description: '关注重点。对话中一律不传（裸读，让论文自己说话）；仅当用户消息里明确点名了具体关注点时，把用户原话作为 focus 传入，不要自行扩展。带方向/透镜/相关论文的复读请引导用户用论文库「精读」面板。' },
        },
        output: {
          schema: {
            type: 'object', additionalProperties: false,
            properties: {
              ...
              jobId: { type: 'string' },
              error: { type: 'string' },
              // 多候选歧义时的匹配列表（不声明会被 additionalProperties:false 拒掉）
              candidates: {
                type: 'array',
                items: {
                  type: 'object', additionalProperties: false,
        ...
        presentCall: (args: any) => callView(`精读${args.light ? '（轻量档）' : ''}：${String(args.paper ?? args.path ?? '').slice(0, 40)}`, { mode: args.mode }),
        presentResult: (args: any, result: any) => {
          const v = result?.value ?? result ?? {};
          if (v.jobId) return resultView('后台精读已启动', `任务 ${String(v.jobId).slice(0, 18)} · ${String(v.label ?? v.mode ?? '')}`.slice(0, 90));
          return resultView(`精读完成：${String(v.title ?? args.paper ?? '').slice(0, 36)}`,
            v.ok ? `已归档 ${v.file ?? ''} · summary${v.summaryUpdated ? '已回填' : '未改'}` : (v.error || '失败'));
        },
        async execute(args: any, exec?: unknown) {
          if (!readEngine) throw new Error('精读引擎不可用（宿主缺少 llm 服务）');
          const store = await getStore();
          const mode = args.mode === 'quick' ? 'quick' : 'paper';
          let absPath = typeof args.path === 'string' ? args.path.trim() : '';
          let paper: import('./shared/types.js').Paper | undefined;
          if (absPath === '') {
            const candidates = findPaperCandidates(store, String(args.paper ?? ''));
            if (candidates.length === 0) return { ok: false, error: `未找到论文：${String(args.paper ?? '')}` };
            if (candidates.length > 1) {
              return { ok: false, error: `“${String(args.paper ?? '')}”匹配到 ${candidates.length} 篇，请用精确 id`, candidates: candidateList(candidates) };
            }
            paper = candidates[0];
            if (!paper.pdfPath) {
              // 无 PDF：arXiv 论文先补下载，其余要求手动上传
              if (paper.arxivId) {
                try {
                  await downloadPaperPdf(paper, store, getConfig().fetchProxy || undefined);
                } catch (err) {
                  return { ok: false, error: 'PDF 补下载失败：' + (err instanceof Error ? err.message : String(err)) };
                }
              } else {
                return { ok: false, error: '该论文没有 PDF，请先在详情页上传' };
              }
            }
            absPath = join(store.dir, paper.pdfPath!);
          }
          const paperRef = paper
          // jobs owner 必须是 Agent（带 .ctx；宿主 bash 工具同款 exec.agent 取法）——
          // 传 exec 本体会让 jobs 服务 scopeOf(owner.ctx) 读 undefined 直接抛
          // "Cannot read properties of undefined (reading 'Symbol(dsh.scope)')"
          const ownerAgent = exec && typeof exec === 'object' && 'agent' in (exec as Record<string, unknown>)
            ? (exec as Record<string, unknown>).agent
            : undefined;
          const outcome: ReadOutcome = await readEngine.read(
            { path: absPath, mode, light: args.light === true, focus: typeof args.focus === 'string' ? args.focus.trim().slice(0, 2400) : '', vlm: getConfig().vlmFigures !== 'off' },
            {
              owner: ownerAgent,
          ...
          if (outcome.kind === 'background') {
            return { ok: true, jobId: outcome.jobId, mode, title: paper?.title ?? absPath };
          }
          // 前台完成（短文）：立即归档 + summary 回填
          if (!paper) {
            // path 直读（如已归档报告）：按文件名前缀关联论文（<safeId>-<ts>.html）
            const base = absPath.replace(/^.*[\\/]/, '').replace(/-[^-]*\.html$/, '');
            paper = [...store.papers.values()].find((p) => safeName(p.id) === base);
          }
          if (!paper) {
      ...
      /* ---------- 10. paper_ask：交互式追问（检索精读章节块 → 带引证回答） ---------- */
      ctx.tools.register(defineTool({
        name: 'paper_ask',
        description:
          '针对一篇已精读过的论文追问（交互式精读）。基于该论文精读时保存的章节块做定向检索，'
          + '回答带【第N页】出处与置信度；片段不足以回答会明说。用户对某篇论文提出具体问题'
          + '（"那个消融对比了什么"/"小目标结果如何"）时调用；论文还没精读过则先用 paper_read。',
        parameters: {
          paper: { type: 'string', required: true, description: '论文 id 或标题关键词' },
          question: { type: 'string', required: true, description: '读者的问题' },
        },
        output: {
          schema: {
            type: 'object', additionalProperties: false,
        ...
        presentCall: (args: any) => callView(`追问：${String(args.question ?? '').slice(0, 40)}`, { paper: args.paper }),
        presentResult: (_args: any, result: any) => {
          const v = result?.value ?? result ?? {};
          const a: any = v.answer;
          if (!v.ok) return resultView('追问失败', v.error || '失败');
          return resultView('追问回答',
            mdList([
              String(a?.answer ?? '').slice(0, 600),
              `- 出处：${(a?.pages ?? []).join('、') || '无'}｜置信度：${a?.confidence ?? ''}${a?.sufficient === false ? '｜⚠ 片段不足' : ''}`,
            ]));
        },
        async execute(args: any) {
          if (!asker) throw new Error('追问引擎不可用（宿主缺少 llm 服务）');
          const store = await getStore();
          const candidates = findPaperCandidates(store, String(args.paper ?? ''));
          if (candidates.length === 0) return { ok: false, error: `未找到论文：${String(args.paper ?? '')}` };
          if (candidates.length > 1) {
          ...
          const session = await loadSession(store.dir, paper.id);
