...
import { createPaper, applyPaperPatch, applyCardPatch, createCard, findDuplicate, findSimilarCards } from '../dist/domain.js';
import { parseArxivId, parseDoi, crossrefToMeta, parseArxivFeed } from '../dist/metadata.js';
import { parseOpenalexWork, arxivFromWork, pickBestWork } from '../dist/related.js';
...
try {
  ...
  // ---- readStatus（阅读状态工作流）----
  check('阅读状态: 创建默认 want', (p1.readStatus ?? 'want') === 'want');
  const p1r = applyPaperPatch(p1, { readStatus: 'reading' });
  check('阅读状态: 补丁流转 reading', p1r.readStatus === 'reading');
  const p1bad = applyPaperPatch(p1, { readStatus: 'bogus' });
  check('阅读状态: 非法枚举被忽略', (p1bad.readStatus ?? 'want') === 'want');
  check('阅读状态: want 命中无字段存量论文', filterPapers([p1, p2], { readStatus: 'want' }).length === 2);
  check('阅读状态: reading 只命中已流转论文', filterPapers([p1r, p2], { readStatus: 'reading' }).length === 1
    && filterPapers([p1r, p2], { readStatus: 'reading' })[0]?.id === p1r.id);
  const pDone = applyPaperPatch(p1r, { readStatus: 'done' });
  check('阅读状态: reading→done', pDone.readStatus === 'done'
    && filterPapers([pDone, p2], { readStatus: 'want' }).length === 1);

  // ---- cites 边（OpenAlex 引用关系进图谱）----
  const gA = { id: p1.id, kind: 'paper', label: p1.title };
  const gB = { id: p2.id, kind: 'paper', label: p2.title };
  const gCite1 = mergeGraph({ nodes: [gA, gB], edges: [] }, { nodes: [], edges: [{ source: p1.id, target: p2.id, kind: 'cites' }] }, 'append', new Set([p1.id, p2.id]));
  check('引用边: mergeGraph 接受 cites', gCite1.edges.length === 1 && gCite1.edges[0].kind === 'cites');
  const gCite2 = mergeGraph(gCite1, { nodes: [], edges: [{ source: p1.id, target: p2.id, kind: 'cites' }] }, 'append', new Set([p1.id, p2.id]));
  check('引用边: 幂等去重', gCite2.edges.length === 1);
  ...
  // ---- evidence（证据摘录）----
  const cEv = applyCardPatch(c1, { evidence: '§3.2 / Fig.3：objectness 全召回支撑表' });
  check('证据: 补丁写入', cEv.evidence === '§3.2 / Fig.3：objectness 全召回支撑表');
  const cEvClr = applyCardPatch(cEv, { evidence: '   ' });
  check('证据: 空白字符串清除', cEvClr.evidence === undefined);
  const cNew = createCard({ title: '证据新建卡', insight: 'x', evidence: '  §1: quote  ' });
  check('证据: 创建时 trim 入库', cNew.evidence === '§1: quote');

  // ---- 卡片进图谱（cardSyncPatch / upsertCard 钩子 / deleteCard 级联）----
  const c3 = createCard({
    title: '图谱同步测试卡', insight: 'x', paperId: p1.id,
    tags: ['频域'], importance: 3, relatedCardIds: [c1.id],
  });
  await store.upsertCard(c3);
  const gCards = store.graph;
  check('卡片图谱: 想法节点入库', gCards.nodes.some((n) => n.kind === 'idea' && n.id === c3.id && n.label === '图谱同步测试卡'));
  check('卡片图谱: derives_from 边', gCards.edges.some((e) => e.source === c3.id && e.target === p1.id && e.kind === 'derives_from'));
  check('卡片图谱: uses 概念边', gCards.edges.some((e) => e.source === c3.id && e.kind === 'uses' && e.target.startsWith('cpt_')));
  check('卡片图谱: related 边', gCards.edges.some((e) => e.kind === 'related' && ((e.source === c3.id && e.target === c1.id) || (e.source === c1.id && e.target === c3.id))));
  // 幂等：重复 upsert 不重复加边
  const edgesBefore = gCards.edges.length;
  await store.upsertCard({ ...c3, importance: 4 });
  check('卡片图谱: 重复 upsert 幂等', store.graph.edges.length === edgesBefore);
  // 改题刷新 label
  await store.upsertCard({ ...c3, title: '图谱同步测试卡改名' });
  check('卡片图谱: 改题刷新节点 label', store.graph.nodes.find((n) => n.id === c3.id)?.label === '图谱同步测试卡改名');
  // 删除级联
  await store.deleteCard(c3.id);
  check('卡片图谱: 删除级联清理', !store.graph.nodes.some((n) => n.id === c3.id)
    && !store.graph.edges.some((e) => e.source === c3.id || e.target === c3.id));
  // 全量 patch（auto-sync 路由语义）：所有卡已在 upsert 时入图 → 不应再产新想法节点/derives_from
  const fullPatch = store.cardSyncPatch([...store.cards.values()], [...store.papers.values()]);
  // ---- 相似卡检测 ----
  const simHit = findSimilarCards([c1, c2], '免训练视频编辑思路（新）');
  check('相似卡: 高相似命中', simHit[0]?.id === c1.id && simHit[0].score >= 0.45);
  check('相似卡: 不相似不命中', findSimilarCards([c1, c2], '完全无关的标题 xyzzy').length === 0);
  check('卡片图谱: 全量 patch 幂等', fullPatch.nodes.filter((n) => n.kind === 'idea').length === 0
    && !fullPatch.edges.some((e) => e.kind === 'derives_from'));
  ...
  // ---- OpenAlex 相关论文解析（纯函数，无网络）----
  const oaW = parseOpenalexWork({
    id: 'https://openalex.org/W2741809807',
    display_name: 'YOLOv7: Trainable bag-of-freebies',
    publication_year: 2023,
    cited_by_count: 1204,
    doi: 'https://doi.org/10.48550/arxiv.2307.08434',
    authorships: [{ author: { display_name: 'Chien-Yao Wang' } }, { author: { display_name: 'Hong-Yuan Mark Liao' } }],
    primary_location: { source: { display_name: 'arXiv.org' }, landing_page_url: 'https://arxiv.org/abs/2307.08434' },
  });
  check('OpenAlex: work 解析', oaW?.openalexId === 'W2741809807' && oaW?.citedBy === 1204 && oaW?.year === 2023);
  check('OpenAlex: arXiv id 从 DOI 提取', oaW?.arxivId === '2307.08434' && oaW?.doi === '10.48550/arxiv.2307.08434');
  check('OpenAlex: arXiv id 从 abs 链接提取（无 DOI）', arxivFromWork({
    id: 'W1', display_name: 'x',
    primary_location: { landing_page_url: 'https://arxiv.org/abs/2501.12345v2' },
  }) === '2501.12345');
  check('OpenAlex: 非 arXiv work 无 arxivId', arxivFromWork({ id: 'W2', display_name: 'y', doi: 'https://doi.org/10.1109/CVPR.2024.00123' }) === undefined);
  check('OpenAlex: 标题精确匹配优先', pickBestWork('YOLOv7: Trainable Bag-of-Freebies', [
    { id: 'W9', display_name: 'Some Other Paper' },
    { id: 'W8', display_name: 'yolov7 trainable bag of freebies' },
  ])?.id === 'W8');
  check('OpenAlex: 无精确命中取第一条', pickBestWork('Totally Different', [{ id: 'W7', display_name: 'Whatever' }])?.id === 'W7');
  check('OpenAlex: 空结果返回 null', pickBestWork('x', []) === null);
  check('OpenAlex: 缺 id/display_name 解析为 null', parseOpenalexWork({ id: 'W3' }) === null);

  const feed = parseArxivFeed('<feed><title>arXiv</title><entry>' +
    '<id>http://arxiv.org/abs/2602.20985v2</id>' +
    '<title>Demo&#45;Det: Synthetic Fixture Title</title>' +
  ...
  check('arXiv feed: 标题实体解码+去空格', feed.title === 'Demo-Det: Synthetic Fixture Title');
