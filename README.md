...
科研读论文的一站式工作台：**从"存论文"到"读透论文"到"沉淀想法"的完整闭环**。库文件(论文/卡片/图谱/会话)全部是本地 JSON/PDF;精读与追问会把论文内容和你的关注点提交给宿主配置的 LLM(本地或云端由 provider 决定),抽图走宿主多模态模型,元数据补全会查询 arXiv/Crossref/OpenAlex。详见 dsh-scholar/README.md 的数据流说明。
