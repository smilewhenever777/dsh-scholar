...
const mdPaper = (p: Record<string, any>): string => {
  ...
  return [`**${p.title}**`, meta, [tags, stars].filter(Boolean).join('  ')]
    .filter(Boolean).join('\n');
};
    ...
    const disposers = [
      ...
      ctx.tools.register(defineTool({
        ...
        async execute(args: any) {
          ...
          const papers = all.slice(0, limit).map((p) => ({
            ...
            tags: p.tags, importance: p.importance, summary: p.summary, source: p.source,
          }));
      ...
      ctx.tools.register(defineTool({
        ...
        async execute(args: any) {
          ...
          return { ok: true, created: true, card: toJson(card), ...(droppedRelatedIds.length ? { droppedRelatedIds } : {}) };
        },
      })),
