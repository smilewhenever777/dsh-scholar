...
import type { CardQuery, CardSort, KnowledgeGraph, PaperQuery, PaperSort, ScholarConfig } from './shared/types.js';
...
import { applyCardPatch, applyPaperPatch, createCard, createPaper, findDuplicate } from './domain.js';
...
export const CardPatchInputSchema = z.object({
  title: z.string(),
  insight: z.string(),
  paperId: z.string(),
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (req.method === 'PUT') {
          const body = JSON.parse(await readBody(req)) as { paperDir?: unknown; defaultTags?: unknown; fetchProxy?: unknown };
      ...
      try {
        ...
        if (!rest) {
          ...
          if (req.method === 'POST') {
            ...
            sendJson(res, 201, { created: true, card });
            return;
          }
          sendJson(res, 405, { error: 'method not allowed' });
          return;
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const store = await getStore();
        const patch = store.tagSyncPatch([...store.papers.values()]);
        const graph = mergeGraph(store.graph, patch, 'append', new Set(store.papers.keys()));
        await store.saveGraph(graph);
        sendJson(res, 200, {
          addedNodes: patch.nodes.length,
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (req.method === 'PUT') {
          ...
          const incoming = {
            nodes: body.nodes as KnowledgeGraph['nodes'],
