...
const GUIDANCE = [
  ...
  '**核心纪律:建主线前必须全面扫描仓库**。当工作区还没有主线、或用户要求「重建/梳理主线」时,'
  + '**禁止**只读一个 plan.md/README 就开始建——必须先 glob 扫描 `*.md`、`docs/**`、`reports/**` 等全部文档,'
  + '按时间顺序读关键进展报告(标题含日期/进展/阶段/Gate 的优先),理解研究的**真实演化路径**,然后再建。'
  + '仓库往往比单个规划文档演进得更快——plan.md 可能是三个月前的,实际已经过了多个 Gate 和方向转折。',
  '何时主动维护(无需用户要求):',
  '**核心纪律:实验必须挂在假设下**。任何 `traj_node_add` 都应该传 `hypothesisId`;不传会自动挂到默认假设并在返回值中收到提示——收到提示时,**立即**用 `traj_hypothesis_add` 创建精确假设,再更新节点挂上去。',
  '**核心纪律:方向变化必须更新目标**。发现研究重心转移(用户说「换个方向」「不再做 X」「重点变成 Y」等)时,**必须**调 `traj_goal_set` 修订目标;旧假设标 superseded。',
  ...
  '- **提出新假设** → `traj_hypothesis_add`(text + track:mainline/branch/detour);先建假设再挂实验。',
